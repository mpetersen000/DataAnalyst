...
The dataset contains over 113,000 loans with 81 variables on each 
...
in 2010.  There is a drop at in Q1 of 2013 followed by fairly  
...
The minimum loan amount pre-2010 and post-2010 remained the same at $1000. 
However, the maximum loan amount increased from $25000 to $35000.  

The mean increased from $6050 to $9190.  Post-2010 there are spikes in number  
of loans at $4000, $10000, and $15000. 

For both pre-2010 and post-2010 data, the distribution is right skewed and a 
small number of loans were greater than $25000.
...
####Analysis of Term
Loans are for 1, 3 or 5 year Terms. Almost 80% of loans are for a 3 year term.
...
Pre-2010, loans could be listed with Proper for Borrowers with a  "Bad" credit 
rating. There were 591 pre-2010 loans listed with a lower credit score of 
"Not Available". 
...
distributed - The median and mean are very close at 700 and 699.
...
Pre-2010 borrowers had an upper credit score median of 659 while post-2010  
...
The Credit Grade wsa assigned to pre-2010 loans at the time the listing went 
live. I could not determine via the Prosper website search how the Credit Grade 
was determined. Credit Grade, from lowest-risk to highest-risk, are labeled AA, 
A, B, C, D, E, HR ("High Risk"), and NC ("No Credit").
...
* the credit score, from an official credit reporting agency
...
historical Prosper data. The score ranges 1-11, with 11 being the best,  
...
to be the case post-2010 where loans consistently have borrower income range 
information.
...
It is surprising that Borrowers are paying a higher rate for a Prosper loan  
...
I defined a factor variable for loan status grouping to simplify analysis.
Loans are "InDefault" if the loan status might affect a borrowers credit 
rating.  Otherwise the loan is in "Good Standing".
...
The prosper loans dataset contains 113937 rows and 81 variables - 70 numeric 
and 26 factor variables.  The dataset met the criteria for tidiness as defined  
...
The main areas I focused on are how the change in Prosper Loans business  
affected their two customer groups - borrowers and lenders. 

* Borrower - What features are of interest and may impact how much it costs 
to borrow money?
* Lender - What features are of interest and may impact paying back the loan?
...
Yes I created several new variables namely:
...
The two graphs show the relationship between Borrower Rating and the  
...
The three graphs compare the loan status to each of the three variables  
...
Rating increases. This plot more in line with what I would expect for a borrower  
loan rating.  
...
the first year of the loan. The univariate Term graph showed almost 80% of 
loans are for three years. As we can see in the chart above, loans with a 
higher prosper score  six or above also had about a 1% default rate. If  
...
the Credit Grade and Prosper Rating. The Credit Grade graph shows a larger 
number of defaults for loan amounts greater than $5000 for Credit Grades B and  
lower.  Higher loan amounts even across credit grades are more likely to  
...
For both plots, I only looked at Loans for $10000 to compare Monthly Loan 
Payments pre-2010 and post-2010.  I took the sqrt of the Monthly Loan Payment 
to adjust for the skewness in the data.  The plot for Monthly Loan Payments 
post-2010 shows that interquartile range for payments across all 
...
Analysis of Monthly Loan Payment was really interesting. Payment amounts 
that a borrower can make each month has a big influence on keeping the loan 
in good standing.
...
Rating is based on Credit Score and the Prosper Score.  If they want to 
...
amounts lower.  The red line is for loans with the lowest prosper score and 
the loans most likely to go bad.  As loan amounts increase it shows an upward
trend for the montlyh payment.  If the payments were lower perhaps the loans 
would be less risky.
...
without increased transparency into how these metrics are calculated.
...
see the medians monthly payment for loans in good standing and loans in default 
drift as the monthly payment rose.  From this chart your can see that a lower 
monthly payment impacts the loan status.
...
the monthly payment is correlated to the Loan Amount. The correlation 
between Monthly Loan Payment and Loan Original Amount when calculated with 
cor.test was .91. 

As loan amounts increase, monthly payments increase. 

Prosper made an adjustment in 2010 to keep payments lower.  It would be 
interesting for Prosper to model increasing the options for Term and Monthly 
payments to further reduce the post-2010 10% default rate.

========================================================

# Reflection

For this exploratory data analysis I decided to use one of the larger data 
sets. There were 81 potential variables to explore. It was easy to get off 
track and look into each of the variables. It took time to figure out which 
variables were important and didn't want to miss an insight. I created numerous 
plots as a result that are not included because they were a dead end. I decided 
to focus on questions I thought Prosper's customers would care about.

I was very surprised at the Peer to Peer business environment pre-2010 that 
led to action from the SEC.  It was necessary to have domain knowledge for Peer 
to Peer Lending  and overview of the business - both provided much needed 
context to what I was seeing in the data.

The interesting models I would like to predict are more complex and beyond 
the scope of an EDA.  For example the Prosper Score is an internal scorecard and 
...
the loan origination date. This isn't helpful for loans that are 3 year or 
...
What went well - my EDA skills have improved. I have a much better 
...
able to use a few tips.  I am excited to take on machine learning.  In the 
future I would like to develop a model that would forecast what type of loans 
are likely to stay in good standing.
